...
bot = telebot.TeleBot('5190273989:AAEhZKAEMU1XSveJiW9f-EVjX8W6G4nG7d8')  # Создаем экземпляр бота


# -----------------------------------------------------------------------
# Функция, обрабатывающая команды Python
@bot.message_handler(commands="start")
def command(message, res=False):
    chat_id = message.chat.id
    bot.send_sticker(chat_id, "CAACAgIAAxkBAAEEmFhibTlQAShTjt_yp_jRmWg5lMEoEgACEQEAAn6I0g_CGQlUEoM0hSQE")
...
def get_messages(message):
    chat_id = message.chat.id
    bot.send_message(chat_id, "Это стикер" + message.content_type)
...
def get_messages(message):
    chat_id = message.chat.id
    bot.send_message(chat_id, "Это аудио " + message.content_type)
...
def get_messages(message):
    chat_id = message.chat.id
    bot.send_message(chat_id, "Это голосовое сообщение " + message.content_type)
...
def get_messages(message):
    chat_id = message.chat.id
    bot.send_message(chat_id, "Это фото" + message.content_type)
...
def get_messages(message):
    chat_id = message.chat.id
    bot.send_message(chat_id, "Это видео" + message.content_type)
...
def get_messages(message):
    ...
    bot.send_message(chat_id, "Это документ" + message.content_type + " (" + mime_type + ")")
...
def get_messages(message):
    chat_id = message.chat.id
    bot.send_message(chat_id, "Это координаты " + message.content_type)
...
def get_messages(message):
    chat_id = message.chat.id
    bot.send_message(chat_id, "Это контакты" + message.content_type)
